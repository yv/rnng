...
re_complex_cat = re.compile(r'\(([^\s\):]+):(\S+) ')
re_deleted_cat = re.compile(r"\(([\.,:]|``|''):\S+ ")
...
def main():
    ...
    for gold_line, sys_line in zip(gold_lines, sys_lines):
        ...
        # workaround for bug where -LRB- would get an LRB edge label
        result = result.replace(':LRB- ', ' ')
        result = result.replace(':RRB- ', ' ')
        result = re_deleted_cat.sub(r'(\1 ', result)
        result = re_complex_cat.sub(r'(\1-\2 ', result)
        print result
